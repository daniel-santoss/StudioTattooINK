...
const Login: React.FC = () => {
  ...
  const { signIn } = useAuth(); // Placeholder for context actions if needed
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Role check is handled by AuthContext via onAuthStateChange
      // Just navigation logic here based on role stored in metadata
      ...
      console.error(error); // Keep logging for debugging
  ...
  return (
    <div className="min-h-[calc(100vh-5rem)] flex items-center justify-center bg-background-dark p-4 relative overflow-hidden">
      ...
      <div className="absolute inset-0 pointer-events-none">
        ...
        <div className="absolute inset-0 bg-[url('https://images.unsplash.com/photo-1598371839696-5c5bb00bdc28?q=80&w=2574&auto=format&fit=crop')] bg-cover bg-center opacity-10 mix-blend-overlay"></div>
      </div>
      ...
      <div className="w-full max-w-md bg-surface-dark border border-border-dark rounded-2xl p-8 shadow-2xl relative z-10 backdrop-blur-sm">
        ...
        {error && (
          <div className="mb-6 p-4 rounded-lg bg-red-500/10 border border-red-500/30 flex items-center gap-3 text-red-500 animate-fade-in">
