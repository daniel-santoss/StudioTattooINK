...
const Booking: React.FC = () => {
    ...
    const handleDateSelect = (_day: number, displayStr: string, _isoDate: string) => {
                            ...
                            {['PROFISSIONAL', 'DATA E TURNO', 'DETALHES', 'CONFIRMAR'].map((label, idx) => {
                                ...
                                return (
                                    <div key={idx} className="flex items-center gap-5">
                                        <div className={`size-[28px] rounded-full flex shrink-0 items-center justify-center text-xs font-black transition-all duration-500 border-[2px] z-20 ${active
                                        ...
                                        <span className={`text-[11px] font-black uppercase tracking-[0.15em] transition-colors duration-300 ${active ? 'text-white' : completed ? 'text-white' : 'text-zinc-600'
                                            }`}>{label}</span>
                                    </div>
                                )
                            })}
                                ...
                                <div className="grid grid-cols-1 gap-4">
                                    {artists.map(artist => (
                                        <div
                                            ...
                                            className={`flex items-center gap-5 p-5 rounded-2xl border cursor-pointer transition-all duration-300 group ${selectedArtist === artist.id
                                        ...
                                        <div className="flex flex-col gap-3 h-full">
                                            {periods.map(period => (
                                                <button
                                                    ...
                                                    className={`flex-1 flex items-center justify-between p-4 rounded-2xl border transition-all duration-300 group ${selectedPeriod === period.id
